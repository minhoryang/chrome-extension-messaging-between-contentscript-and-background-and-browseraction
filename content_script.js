...
const onExtensionMessage = (message, sender, sendResponse) => {
	...
	document.dispatchEvent(new CustomEvent('onMyExtensionMessage', { detail: message }));
}
...
document.addEventListener('onMyWebpageMessage', function (e) {
    const data = e.detail;
    if (!data) return;
	chrome.runtime.sendMessage({ type: 'export_response', source: 'content_script.js', data });
});
